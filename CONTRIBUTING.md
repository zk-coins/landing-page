...
This repo is a **static multilingual landing page**: one home page per language (`index.html`, `de/`, `fr/`, `it/`, `es/`) and one zkBTC page per language (`zkbtc/`, `de/zkbtc/`, `fr/zkbtc/`, `it/zkbtc/`, `es/zkbtc/`), shared `/styles.css`, no runtime JavaScript, no build step for the deployed site. Keep it that way unless you have a very good reason.

Locale pages are generated from `scripts/i18n/` (`page.template` for the homes, `zkbtc.template` for the zkBTC pages, plus `strings/*.json`). After editing strings, run:
...
| `npm run i18n:generate` | Rebuild every locale page (homes + zkBTC) and the sitemap from `scripts/i18n/` |
