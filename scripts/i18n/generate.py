...
# WALLET_LINKS_ENABLED stays False and hrefs are omitted (markup kept).
# Flip the flag to restore every wallet link without rewriting the template.
WALLET_URL = "https://zkcoins.app"
WALLET_LINKS_ENABLED = False


def wallet_open(class_name: str = "") -> str:
    """Open a wallet CTA. Href is omitted while WALLET_LINKS_ENABLED is False."""
    cls = f' class="{class_name}"' if class_name else ""
    # Do not put WALLET_URL in HTML comments: lychee would still fetch it.
    comment = "<!-- wallet CTA: href omitted while WALLET_LINKS_ENABLED is False -->"
    if WALLET_LINKS_ENABLED:
        return f'{comment}<a{cls} href="{WALLET_URL}">'
    return f'{comment}<a{cls} aria-disabled="true">'
